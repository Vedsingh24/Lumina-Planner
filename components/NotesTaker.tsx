...
import { Note, DraggableImage, Task } from '../types';
...
const NotesTaker: React.FC<NotesTakerProps> = ({
  ...
  const availableDates = useMemo(() => Array.from(new Set(notes.map(n => n.date).filter(Boolean))).sort() as string[], [notes]);
  ...
  const handlePrevNote = () => {
    ...
    if (currentIndex > 0) {
      ...
      const prev = availableDates.slice().reverse().find(d => d < selectedDate);
      if (prev) onSelectDate(prev);
    }
  };
  ...
  // ─── Document Style (NEVER changes with zoom) ─────────────────────
  const handwrittenStyle: React.CSSProperties = {
    fontFamily: isCursive ? '"Dancing Script", cursive' : '"Inter", sans-serif',
    color: '#38bdf8',
    lineHeight: String(LINE_HEIGHT_MULT),
    letterSpacing: isCursive ? '0.04em' : 'normal',
    fontSize: isCursive ? `${FONT_SIZE_REM}rem` : `${FONT_SIZE_REM - 0.2}rem`,
  ...
  return (
    <div className="flex flex-col h-full bg-[#020617] relative animate-in fade-in duration-500 rounded-2xl border border-white/5 overflow-hidden">
      ...
      <div className="flex justify-between items-center px-4 py-2 border-b border-white/5 glass-panel z-50 relative">
        <div className="flex items-center gap-3 relative">
          ...
          <div className="flex items-center bg-slate-900 border border-white/10 rounded-lg shadow-sm relative">
            ...
            <button
              ...
              className="px-3 py-1 bg-blue-500/10 hover:bg-blue-500/20 transition-colors border-x border-white/5 text-xs font-bold tracking-widest text-blue-400 uppercase cursor-pointer flex items-center gap-1.5"
            >
              {selectedDate}
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            className="px-3 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded text-xs font-semibold transition-colors border border-white/5 mr-2 flex items-center justify-center gap-2 h-8"
          >
            {isCursive ? (
               <span className="font-[Dancing_Script] text-sm tracking-wide">Cursive</span>
            ) : (
               <span className="font-sans text-xs">Standard</span>
            )}
          </button>
          <div className="flex items-center bg-slate-800 rounded-lg p-1 border border-white/5 mr-4 h-8">
          ...
          <button onClick={() => { setExportDates([selectedDate]); setIsExportModalOpen(true); }} className="px-4 py-1.5 h-8 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-sm font-semibold transition-colors flex items-center gap-2 border border-white/5 shadow-lg shadow-black/20">
        ...
        >
          ...
          <div className="w-full md:w-1/2 bg-slate-950 relative overflow-hidden flex flex-col">
            ...
            <div
              ref={measureRef}
              aria-hidden="true"
              className={`ql-editor preview-measure ${isCursive ? 'cursive-mode' : 'standard-mode'}`}
              style={{
                position: 'absolute',
                visibility: 'hidden',
                pointerEvents: 'none',
              ...
              {pages.map((pageIdx) => {
                ...
                return (
                  <div
                    key={pageIdx}
                    style={{
                      width: `${A4_W * (zoom * 0.70)}px`,
                      height: `${A4_H * (zoom * 0.70)}px`,
                      flexShrink: 0,
                      position: 'relative',
                    }}
                  >
                    ...
                    >
                      ...
                      <div style={{
                        ...
                        */}
                        <div
                          className={`ql-editor ${isCursive ? 'cursive-mode' : 'standard-mode'}`}
                          style={{
                            ...handwrittenStyle,
                            padding: 0,
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        /* ─── Preview: Headings ─────────────────────────────────────────── */
        .cursive-mode h1, .cursive-mode h2, .cursive-mode h3, .cursive-mode p, .cursive-mode li {
          font-family: 'Dancing Script', cursive;
        }
        .standard-mode h1, .standard-mode h2, .standard-mode h3, .standard-mode p, .standard-mode li {
